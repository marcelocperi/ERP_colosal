...
def setup_cost_accounting_permission():
    ...
    with get_db_cursor(dictionary=True) as cursor:
        ...
        # 2. Gestionar Roles
        cursor.execute("SELECT id, name FROM sys_roles WHERE name LIKE %s OR name = %s", ('%Admin%', 'ANALISTA_COSTOS'))
        ...
        # La tabla es sys_role_permissions (singular role)
        for role in roles:
            cursor.execute("SELECT 1 FROM sys_role_permissions WHERE role_id = %s AND permission_id = %s", (role['id'], perm_id))
            if not cursor.fetchone():
                print(f"Asignando cost_accounting al rol: {role['name']}")
                cursor.execute("INSERT INTO sys_role_permissions (role_id, permission_id) VALUES (%s, %s)", (role['id'], perm_id))
            else:
